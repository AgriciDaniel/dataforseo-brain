...
Installs the DataForSEO Brain skill surface.
Use --target custom --path <agent-skill-root> for Hermes or another runtime
when its official skill root is known.
Set DATAFORSEO_BRAIN_INSTALL_HOME to test against a temporary home directory.
USAGE
}
...
base_home="${DATAFORSEO_BRAIN_INSTALL_HOME:-${HOME}}"

target_dir() {
  case "$1" in
    codex) echo "${base_home}/.codex/skills/dataforseo-brain" ;;
    claude) echo "${base_home}/.claude/skills/dataforseo-brain" ;;
    agents) echo "${base_home}/.agents/skills/dataforseo-brain" ;;
    openclaw) echo "${base_home}/.openclaw/skills/dataforseo-brain" ;;
    portable) echo "${base_home}/.agent-skills/dataforseo-brain" ;;
    custom) echo "${custom_path%/}/dataforseo-brain" ;;
  esac
}
...
install_one() {
  ...
  chmod -R go-rwx "${dest}"
  echo "DataForSEO Brain installed to ${dest}"
}

install_gemini() {
  local dest="${base_home}/.gemini/dataforseo-brain"
  local loader="${base_home}/.gemini/GEMINI.md"
  install_one "${dest}"
  mkdir -p "$(dirname "${loader}")"
  python - "${loader}" "@./dataforseo-brain/GEMINI.md" <<'PY'
...
start = "<!-- dataforseo-brain-install:start -->"
end = "<!-- dataforseo-brain-install:end -->"
block = f"{start}\n{import_line}\n{end}"
...
PY
  echo "DataForSEO Brain Gemini loader updated at ${loader}"
}
