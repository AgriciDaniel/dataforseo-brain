...
def main() -> int:
    run(["-m", "compileall", "scripts", "dataforseo_brain", "tests"])
    run(["scripts/lint_vault.py", "--vault", "assets/template-brain", "--template"])
    with tempfile.TemporaryDirectory(prefix="dataforseo-brain-test-") as tmp:
    ...
    with tempfile.TemporaryDirectory(prefix="dataforseo-brain-install-") as tmp:
        env = {"DATAFORSEO_BRAIN_INSTALL_HOME": tmp}
        run_cmd(["bash", "install.sh", "--target", "all"], env=env)
        assert (Path(tmp) / ".codex" / "skills" / "dataforseo-brain" / "SKILL.md").exists()
        assert (Path(tmp) / ".openclaw" / "skills" / "dataforseo-brain" / "SKILL.md").exists()
        assert (Path(tmp) / ".agent-skills" / "dataforseo-brain" / "SKILL.md").exists()
        assert (Path(tmp) / ".gemini" / "dataforseo-brain" / "GEMINI.md").exists()
        assert "dataforseo-brain-install:start" in (Path(tmp) / ".gemini" / "GEMINI.md").read_text(encoding="utf-8")
        custom_root = Path(tmp) / "custom-skills"
        run_cmd(["bash", "install.sh", "--target", "custom", "--path", str(custom_root)], env=env)
        assert (custom_root / "dataforseo-brain" / "SKILL.md").exists()
        run_cmd(["bash", "uninstall.sh", "--target", "all"], env=env)
        assert not (Path(tmp) / ".codex" / "skills" / "dataforseo-brain").exists()
        assert not (Path(tmp) / ".gemini" / "dataforseo-brain").exists()
        assert not (Path(tmp) / ".gemini" / "GEMINI.md").exists()
        run_cmd(["bash", "uninstall.sh", "--target", "custom", "--path", str(custom_root)], env=env)
        assert not (custom_root / "dataforseo-brain").exists()
